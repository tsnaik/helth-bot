...
export default (ctx) => {
  ...
  const appendedText = ctx.message.text.substring(entity.length - entity.offset + 1);
  console.log(appendedText)

  if (!appendedText) {
      ctx.reply(`Please provide a time after ${ctx.message.text}. Possible options are: ${allowList.join(', ')}. \n\nExample: Send "/helth today" to record today's activity.`)
      return
  }
  ...
  try {
      ...
      if (isOk) {
          ctx.reply(`${ctx.message.from.first_name}, I have recorded your entry for ${appendedText}. Kudos for getting it done! 🎉`)
      }
  } catch (error) {
